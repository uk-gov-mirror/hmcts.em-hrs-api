...
//        when(this.hearingRecordingRepository.findById(any(UUID.class))).thenReturn(Optional.of(TestUtil.STORED_DOCUMENT));
...
//        when(this.hearingRecordingRepository.findById(any(UUID.class))).thenReturn(Optional.empty());
//        Optional<HearingRecording> hearingRecording = hearingRecordingService.findOneWithBinaryData(TestUtil.RANDOM_UUID);
//        assertFalse(hearingRecording.isPresent());
//    }
//
//    @Test
//    public void testFindOneWithBinaryDataThatIsMarkedHardDeleted() {
//        when(this.hearingRecordingRepository.findById(any(UUID.class))).thenReturn(Optional.of(HARD_DELETED_DOCUMENT));
//        Optional<HearingRecording> hearingRecording = hearingRecordingService.findOneWithBinaryData(TestUtil.RANDOM_UUID);
//        assertFalse(hearingRecording.isPresent());
//    }
//
...
//
//        ArgumentCaptor<DocumentContentVersion> captor = ArgumentCaptor.forClass(DocumentContentVersion.class);
//        verify(blobStorageWriteService).uploadDocumentContentVersion(hearingRecording, documentContentVersion, TEST_FILE);
//        verify(documentContentVersionRepository).save(captor.capture());
//        assertThat(captor.getValue(), is(documentContentVersion));
//    }
//
...
//
//        assertThat(hearingRecordingWithContent.getMostRecentDocumentContentVersion().getDocumentContent(), nullValue());
...
//
//        final DocumentContentVersion latestVersionInFolder = folder.getHearingRecordings().get(0).getDocumentContentVersions().get(0);
//
...
//        verify(securityUtilService).getUserId();
//        verify(folderRepository).save(folder);
//        verify(blobStorageWriteService).uploadDocumentContentVersion(folder.getHearingRecordings().get(0), latestVersionInFolder, TEST_FILE);
//    }
//
//    @Test
...
//
//        DocumentUpdate update = new DocumentUpdate(hearingRecording.getId(), Maps.newHashMap("UpdateKey", "UpdateValue"));
...
//
//        Date newTtl = new Date();
//        hearingRecordingService.updateHearingRecording(hearingRecording, newTtl, Maps.newHashMap("UpdateKey", "UpdateValue"));
//
//        assertEquals(newTtl, hearingRecording.getTtl());
//        assertEquals(hearingRecording.getMetadata().get("Key"), "Value");
//        assertEquals(hearingRecording.getMetadata().get("UpdateKey"), "UpdateValue");
